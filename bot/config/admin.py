...
def get_phones(is_all=True) -> tuple:
    phone_list = tuple
    if os.path.isfile(settings.admin.allowed_users_file):
...
def save_phones(phone_list: list) -> bool:
    if os.path.isfile(settings.admin.allowed_users_file):
        with open(settings.admin.allowed_users_file, "w") as file:
            file.writelines("\n".join(phone_list))
            file.write("\n")
        return True
...
def add_phone(phone_number: str) -> bool:
    if os.path.isfile(settings.admin.allowed_users_file):
        with open(settings.admin.allowed_users_file, "a") as file:
