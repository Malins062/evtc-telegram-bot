...
from bot.config.admin import add_phone, get_phones, save_phones
from bot.filters.is_admin import IsAdminUser
from bot.states.user_states import UserStates
from bot.validators.admin import validate_phone_number
...
@router.message(
    UserStates.add_phone,
    F.text.cast(validate_phone_number).as_("phone_number"),
    IsAdminUser(),
)
async def handle_add_phone_number(
    ...
    if phone_number in phone_list:
        await message.reply(
            text="😯 Номер телефона уже есть в списке разрешенных.",
        ...
        if add_phone(phone_number):
            ...
            text_msg = (
                "К сожалению, номер указанный телефона не добавлен список разрешенных."
            )
            logger.warning(text_msg)

        await message.reply(text=text_msg, reply_markup=types.ReplyKeyboardRemove())

    await handle_card(message, state)


@router.message(UserStates.remove_phone, F.text, IsAdminUser())
async def handle_remove_phone_number(message: types.Message, state: FSMContext):
    await state.update_data(remove_phone=True)

    phone_list = get_phones(is_all=False)
    if F.text not in phone_list:
        await message.reply(
        ...
        if save_phones(phone_list):
            ...
            text_msg = (
                "К сожалению, указанный номер телефона не удален из списка разрешенных."
            )
            logger.warning(text_msg)
